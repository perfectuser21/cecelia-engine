...
- **skills/dev/steps/00-worktree-auto.md**: 修复僵尸 `.dev-mode` 检测逻辑过于激进的问题
  - **问题**：当 `started` 字段无法解析时，直接判定为僵尸并删除文件，导致第二个会话无法检测到第一个会话，未创建 worktree 隔离
  - **根因**：`started` 字段未展开（如 `$(date ...)` 模板未替换）时，`date -d` 解析失败返回 0，导致年龄计算错误
  - **修复**：多条件综合判断，使用文件修改时间作为主要依据，`started` 字段解析失败时 fallback 到文件时间，同时检查分支活跃度
  - **测试**：新增 RCI 节点 W6-001-1/2/3 覆盖边界场景
